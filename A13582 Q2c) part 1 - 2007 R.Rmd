---
title: "Question 2c) - part 1 YEAR 2007"
...
" ******"running each chunk separately to get a faster output"******
```{r}
library(readr)
...
*"Loading data set 2007"*
```{r}
dataset_2007 <- read.csv("C:/Users/user/Desktop/progdata/data2007.csv")
head(dataset_2007)
```

*"Remove columns with missing values and other unnecessary columns"*
```{r}
colSums(is.na(dataset_2007))
dataset_2007 <- subset(dataset_2007, select = -c(CancellationCode, AirTime, ArrDelay, ArrTime, TailNum))
```
...
```{r}
data_origin <- merge(dataset_2007, airports_dataset, by.x='Origin', by.y='iata', all.x=TRUE, suffixes=c('_origin', '_dest'))
...
```{r}
diverted_counts <- c(5088955, 111539)
labels <- c("Not Diverted", "Diverted")
pie(diverted_counts, labels = labels, main = "Percentage of Flights Diverted - 2007", col = c("skyblue", "lightgreen"))
...
pie(diverted_count, labels = labels_2, main = "Percentage of Flights Diverted - 2007", col = c("skyblue", "lightgreen"))
...
print(paste("Accuracy - 2007:", round(accuracy, 4)))
print(paste("F1 Score - 2007:", round(f1_score, 4)))
```
...
cat("Classification Report - 2007:\n")
